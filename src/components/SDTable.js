...
import { isBoolean, isNumber, isString, getNumber } from "../tools/utils";
...
export default class SDTable extends React.Component {
    ...
    generateColumns = (props) => {
        ...
        if (!contentEl || !tableEl || len < 1 || !scroll) {
        ...
        if (scrollY || scroll.x) {
            ...
            source.forEach((data) => {  // 累计份数
                const count = getNumber(data.width) || 0;
