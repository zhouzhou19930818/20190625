...
class ManagerListRoute extends Component {
            ...
            render: (d) => {
                return [
                    ...
                    <Button htmlType="button"
                            key="op_2"
                            style={ { height: '26px', marginRight: '3px' } }
                            onClick={ () => this.setState({ isCreate: false },
                                () => this.props.history.push(prefixRoute + '/fs_file_create', { record: d })) }
                    >编辑</Button>,
                    <Button htmlType="button"
                            key="op_3"
                            style={ { height: '26px' } }
    ...
    getList = () => {
        ...
        api.getAllKs().then(res => {
            ...
            this.nameList = resData && resData.map((d) => d.ksName);
            this.setState({
                tableLoading: false,
                dataSource: resData && resData.map((d, i) => ({ ...d, index: i + 1 })),
            });
        }).catch(() => {
            this.setState({ tableLoading: false });
        });
                ...
                <div className="sd-filter-form">
                    <Button
                        ...
                        onClick={ () => this.setState({ isCreate: true },
                            () => {
                                this.props.history.push(prefixRoute + '/fs_file_create', {nameList: this.nameList});
                            }) }
                    >新增文件</Button>
                    <Input.Search
