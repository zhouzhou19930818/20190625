...
export const getTotalTooltip = (params, seriesSource, type) => {
    ...
    const source = seriesSource[seriesIndex];
    const sumSource = seriesSource[0];
    if (!source || !sumSource) return '';
    const obj = source.series[dataIndex];
    const sumObj = sumSource.series[dataIndex];
    if (!obj || !sumObj) return '';
    const appName = obj.appName || source.appName;
    return type === 'memory' ?
        `时间：${ obj.time }<br/>
         应用名称：${ appName && appName !== 'null' ? appName : '' }<br/>
         总内存使用量：${ sumObj.allAppTotalMemory }<br/>
         内存使用量：${ obj.totalMemory || obj.allAppTotalMemory }<br/>
         CPU个数：${ obj.totalCpu || obj.allAppTotalCpu }` :
        `时间：${ obj.time }<br/>
         应用名称：${ appName && appName !== 'null' ? appName : '' }<br/>
         总CPU使用量：${ sumObj.allAppTotalCpu }<br/>
         CPU个数：${ obj.totalCpu || obj.allAppTotalCpu }<br/>
         内存使用量：${ obj.totalMemory || obj.allAppTotalMemory }`
};
