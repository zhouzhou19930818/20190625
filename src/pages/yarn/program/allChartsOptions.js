...
export const getTotalTooltip = (params, seriesSource, type) => {
    ...
    return type === 'memory' ?
        `时间：${ obj.time }<br/>
         内存使用量占比：${ obj.memoryRate ? (obj.memoryRate * 100).toFixed(2) : 0 }%<br/>
         内存使用量: ${ obj.memoryUsed }<br/>
         磁盘使用空间占比：${ obj.diskRate ? (obj.diskRate * 100).toFixed(2) : 0 }%<br/>
         程序运行中/待分配: ${ obj.runningAppCount }/${ obj.acceptedAppCount }<br/>
         程序个数：${ obj.appCount }<br/>
         CPU个数：${ obj.cpuUsed }` :
        `时间：${ obj.time }<br/>
         CPU使用量占比：${ obj.cpuRate ? (obj.cpuRate * 100).toFixed(2) : 0 }%<br/>
