...
export default class Program extends Component {
        ...
        api.getProgramCpuAndMemoryTrend({
            ...
            .then(res => {
                ...
                if (response.success !== 'true' || !response.data) {
                    response.success !== 'true' && message.error('获取集群内存、CPU趋势失败: ' + response.msg);
                    errorHandler();
                    return;
                }
    ...
    getMemoryTopNTrend = ({ cluster, startTime, endTime } = {}) => {
        ...
        api.getTopProgramMemory(params)
            .then(res => {
                const response = res.data;
                const data = response.data || [];
                if ((!data.programInfos || !data.programInfos[0])) {
                    response.success !== 'true' && message.error('获取区间内内存TOPN程序趋势失败: ' + response.msg);
                    errorHandler();
                    return;
                }
    ...
    getOverRunningTimeProgram = (cluster, overRunningTime) => {
        api.getOverRunningTimeProgram({
            ...
            const response = res.data;
            if (response.success !== 'true' || !response.data) {
                response.success !== 'true' && message.error('获取程序运行时长失败: ' + response.msg);
                this.updateState({
                    overRunningTimeTable: {
                        loading: { $set: false },
                        dataSource: { $set: [] },
    ...
    refresh = (startTime, endTime) => {
        this.updateState({
            ...
            this.getMemoryCpuTrend({
                startTime: startTime || this.totalStartTime,
                endTime: endTime || this.totalEndTime
            });
        });
    };
    ...
    onTotalDatePickerChange = (attr) => (obj, time) => {
        ...
        if (this.customTotalStartTime && this.customTotalEndTime) {
            ...
            this.refresh(this.customTotalStartTime, this.customTotalEndTime);
        }
    };
